...
print '<body>';
my $Enrichment_CMD = "R --vanilla --slave --args $id $idtype $months $subtype $split $tamoxifen $chemo $surv $user_result_dir< ../resources/scripts/BRCADB_2013_plotkmfxn_perl.R ";
...
##zip the files
   ...
   # Add a directory tree
    $zip->addTree( $user_result_dir."/".$id."-"."survival/", $id."-"."Survival", sub { -f && -r } );
   
   # Save the Zip file
   unless ( $zip->writeToFileNamed($user_result_dir."/".$id."-"."Survival.zip") == AZ_OK ) {
       die 'write error';
   }
...
my $zipfileloc = "../results"."/".$user_id."/".$id."-"."Survival.zip";
