...
my ( $name, $path, $extension ) = fileparse ( $forestfile, '\..*' );
	...
	$forestfile =~ tr/ /_/;
	$forestfile =~ s/[^$safe_filename_characters]//g;
	
	if ( $forestfile =~ /^([$safe_filename_characters]+)$/ )
	{
	$forestfile = $1;
	}
	else
	...
	open ( UPLOADFILE, ">$upload_dir/$forestfile" ) or die "$!";
	binmode UPLOADFILE;
...
my $lines=0;
open (FILE, "<$upload_dir/$forestfile") or die "Can't open file: $!";
$lines++ while (<FILE>);
close FILE;
if (scalar($lines) > 50 ) {
...
#zip the files
   ...
   # Save the Zip file
   unless ( $zip->writeToFileNamed($user_result_dir."/".$forestlabel."-"."forestplot.zip") == AZ_OK ) {
       die 'write error';
   }
